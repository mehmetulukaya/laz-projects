...
begin
  ...
  h:=image1.height;
  w:=image1.width;
  // queryperformancefrequency(freq);  // todo find some functions for cross compiling
  Randomize;
  Caption:='Molecules';
  dt:=0.1;
  setup(4);
...
begin
  ...
  repeat
    i:=1;                { Main loop }
    // queryperformancecounter(nextstop);  // todo find some functions for cross compiling
    repeat
      molecules[i].move;
      molecules[i].reflect;
      Collisions;
      // repeat
        // queryperformancecounter(stopcount);  // todo find some functions for cross compiling
...
begin
  ...
  for i:=2 to n do
  with molecules[i] do
    if col=bgc then col:=RGBToColor(random(256), random(256), random(256))
    else col:=bgc;
end;
