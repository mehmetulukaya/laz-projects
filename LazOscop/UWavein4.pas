...
type
  ...
  TWaveIn = class(TObject)
    ...
    Function GetDeviceCaps:Boolean;
...
begin
  ...
  begin
    ...
    nBlockAlign := 1;
    wBitsPerSample := 8;
    nAvgBytesPerSec := nSamplesPerSec*(wBitsPerSample div 8);
    cbSize := 0;
  end;
  { Success, go home }
  Result := True;
...
{********** GetDeviceCaps *************}
function TWaveIn.GetDeviceCaps:boolean;
var mmerror:word;
begin
  new(PtrWaveInCaps);
  If (ptrWaveInCaps = Nil) Then
  begin
    fposterror('Error allocating space for device capabilities');
    Result := False;
    Exit;
  end;
  { initialize the format to standard PCM }
  {ZeroMemory( lptrwavefmtex, maxFmtSize );}
  mmerror:=WaveInGetDevCaps(0,ptrWaveincaps, sizeof(ptrWaveinCaps^));
  if mmerror<>0 then
  begin
    fPosterror('Error getting device capabilties');
    result:=false;
    exit;
  end;
  result:=true;  
...
begin
  ...
  InitWaveIn:=False; {false while allocatiing parameters}
  if(Not AllocWaveFormatEx) Then  exit;
  {find a device compatible with the available wave characteristics }
  i:=waveInGetNumDevs;
  If i < 1  Then
  begin
    fposterror('No wave audio input devices found');
    exit;
  end;
  {allocate the wave header memory }
  if (Not AllocWaveHeaders) Then exit;
  {allocate the wave data buffer memory }
  if (Not AllocPCMBuffers)  Then Exit;
  {get device capabilities}
  If not GetDeviceCaps then exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      fposterror(format('%2d: ',[ierr])+'Error in waveInUnprepareHeader ' + s);
    end;
  end;
...
begin
  ...
  if (iErr <> 0) then fposterror(format('%2d: ',[ierr])+'Error in waveInReset');
  CloseWaveDevice;
end;
...
begin
  ...
  begin
    StopInput;
    fposterror(format('%2d: ',[ierr])+'Error adding buffer, recording stopped: ' + GetErrorText(iErr));
    result:=FALSE;
    exit;
  end;
...
begin
  ...
  begin
    CloseWaveDevice;
    fposterror(format('%2d: ',[ierr])+'Error starting wavein, close attempted: ' + GetErrorText(iErr));
  end
  else
  begin
    RecordActive := TRUE;
    ...
    begin
      fposterror('System error: AddNextBuffer failed from open');
      exit;
    end;
    result:=True;
  end;
...
begin
  ...
  *)
  ...
  begin
    ...
    fposterror(format('%2d: ',[ierr])+ 'Could not open the input device for display: ' {+ ^M}
                   +' '+ GetErrorText(iErr));
    result:=FALSE;
    exit;
  end;
  ...
  begin
    ...
    begin
      CloseWaveDevice;
      fposterror(format('%2d: ',[ierr])+'Error preparing header for recording: ' + ^M +
                          GetErrorText(iErr));
      result:=FALSE;
      Exit;
    end;
