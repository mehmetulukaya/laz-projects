...
type
 ...
 dialog = object
   ...
   constructor Construct(appl : the_application_ptr; info : char_ptr; clr : aggclr_ptr = NIL );
   destructor  Destruct;

   procedure set_waiting(act : func_action );

   procedure add_action(name : char_ptr; act : func_action; x1 ,y1 ,x2 ,y2 : double );
   procedure add_choice(name ,attr : char_ptr; x ,y : double; status : boolean = false );

   procedure change_text(text : char_ptr; x ,y : double; clr : aggclr_ptr = NIL );
   procedure append_text(text : char_ptr );
 ...
 the_application = object(platform_support )
   ...
   procedure draw_text(x ,y : double; msg : char_ptr; clr : aggclr_ptr = NIL );
