...
//-----------------------------------------------font_engine_win32_tt_base
 font_engine_win32_tt_base = object(font_engine )
  ...
  // Set font parameters
   ...
   function  create_font (typeface_ : char_ptr; ren_type : glyph_rendering ) : boolean;
  ...
  // Private
   ...
   function  find_font(name : char_ptr ) : int;
...
begin
 ...
 m_typeface.size:=sizeof(char8 ) * (256 - 16 );

 agg_getmem(pointer(m_typeface.name ) ,m_typeface.size );

 m_typeface_len:=256 - 16 - 1;

 m_signature.size:=sizeof(char8 ) *  (256 + 256 - 16 );
  ...
  begin
   ...
   if len > m_typeface_len then
    begin
     ...
     m_typeface.size :=sizeof(char8 ) * (len + 32 );
     m_signature.size:=sizeof(char8 ) * (len + 32 + 256 );
   ...
   StrCopy(char_ptr(m_typeface.name ) ,typeface_ );
   ...
   idx:=find_font(char_ptr(m_signature.name ) );
    ...
    begin
     m_cur_font:=
      CreateFont(
       ...
       char_ptr(m_typeface.name )  // typeface name
       );
     ...
     if m_cur_font <> 0 then
      begin
       ...
       n.size:=sizeof(char8 ) * (StrLen(char_ptr(m_signature.name ) ) + 1 );

       agg_getmem(pointer(n.name ) ,n.size );
       StrCopy   (char_ptr(n.name ) ,char_ptr(m_signature.name ) );
...
begin
 result:=char_ptr(m_typeface.name );
...
begin
 ...
 if (m_dc <> 0 ) and
  ...
  begin
   ...
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%u,'  ,m_char_set );
   ...
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,m_weight );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,int(m_italic ) );
   ...
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,int(m_pitch_and_family ) );
