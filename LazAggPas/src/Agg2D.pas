...
{$IFDEF AGG2D_USE_FREETYPE }
 if cache = AGG_VectorFontCache then
  m_fontEngine.load_font(char_ptr(@fileName[1 ] ) ,0 ,glyph_ren_outline )
 else
  m_fontEngine.load_font(char_ptr(@fileName[1 ] ) ,0 ,glyph_ren_agg_gray8 );
...
{$ELSE }
 ...
 if cache = AGG_VectorFontCache then
  m_fontEngine.create_font_(char_ptr(@fileName[1 ] ) ,glyph_ren_outline ,height ,0.0 ,b ,italic )
 else
  m_fontEngine.create_font_(char_ptr(@fileName[1 ] ) ,glyph_ren_agg_gray8 ,worldToScreen(height) ,0.0 ,b ,italic );
...
begin
 ...
 while char_ptr(ptrcomp(str_ ) + i * sizeof(char8 ) )^ <> #0 do
  begin
   glyph:=m_fontCacheManager.glyph(int32u(char_ptr(ptrcomp(str_ ) + i * sizeof(char8 ) )^ ) );
