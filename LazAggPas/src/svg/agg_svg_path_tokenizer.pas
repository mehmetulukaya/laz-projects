...
// returns the last recognized numeric value, so does last_command().
 ...
 path_tokenizer = object
   m_separators_mask ,
   m_commands_mask   ,
   m_numeric_mask    : array[0..256 div 8 - 1 ] of char8;

   m_path         : char_ptr;
   m_last_number  : double;
   m_last_command : char8;
   ...
   function  next(cmd : char8 ) : double; overload;

   function  last_command : char8;
  ...
  begin
   if not is_separator(unsigned(m_path^ ) ) then
    begin
     ...
     raise svg_exception.Construct(char_ptr(@buf[0 ] ) );
...
{ NEXT }
function path_tokenizer.next(cmd : char8 ) : double;
var
 buf : array[0..99 ] of char8;

begin
 if not next then
  raise svg_exception.Construct(char_ptr('parse_path: Unexpected end of path' ) );
...
var
 buf : array[0..255 ] of char8; // Should be enough for any number
...
// Copy all numeric characters
 ...
 m_last_number:=get_double(pointer(char_ptr(@buf[0 ] ) ) );
