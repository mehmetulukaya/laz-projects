 ...
 while tok.next do
  begin
   ...
   case cmd of
    ...
    'A' ,'a' :
     raise svg_exception.Construct(char_ptr('parse_path: Command A: NOT IMPLEMENTED YET' ) );
...
begin
 if m_attr_storage.size = 0 then
  raise svg_exception(char_ptr('end_path : The path was not begun' ) );
...
begin
 if m_attr_stack.size = 0 then
  raise svg_exception(char_ptr('pop_attr : Attribute stack is empty' ) );
...
begin
 if m_attr_stack.size = 0 then
  raise svg_exception.Construct(char_ptr('cur_attr : Attribute stack is empty' ) );
