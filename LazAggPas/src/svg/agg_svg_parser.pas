...
begin
 ...
 if StrComp(char_ptr(el ) ,'title' ) = 0 then
  this.m_title_flag:=true
 else
  if StrComp(char_ptr(el ) ,'g' ) = 0 then
   begin
   ...
   if StrComp(char_ptr(el ) ,'path' ) = 0 then
    begin
     if this.m_path_flag then
      raise svg_exception.Construct(char_ptr('start_element: Nested path' ) );
    ...
    if StrComp(char_ptr(el ) ,'rect' ) = 0 then
     this.parse_rect(attr )
    else
     if StrComp(char_ptr(el ) ,'line' ) = 0 then
      this.parse_line(attr )
     else
      if StrComp(char_ptr(el ) ,'polyline' ) = 0 then
       this.parse_poly(attr ,false )
      else
       if StrComp(char_ptr(el ) ,'polygon' ) = 0 then
        this.parse_poly(attr ,true );

     //else
     // if StrComp(char_ptr(el ) ,'<OTHER_ELEMENTS>' ) = 0 then
     //  begin
     //  end
     //...
 ...
 if str^ = '#' then
  ...
  begin
   ...
   for u:=0 to colors_num - 1 do
    if StrComp(colors[u ].name ,char_ptr(str ) ) = 0 then
     begin
      p:=@colors[u ];
   ...
   if p = NIL then
    begin
     m:='parse_color: Invalid color name ' + StrPas(char_ptr(str ) ) + #0;

     raise svg_exception.Construct(char_ptr(@m[1 ] ) );
...
begin
 ...
 result:=get_double(pointer(char_ptr(str ) ) );

end;

{ islower }
function islower(ch : char8 ) : boolean;
begin
 case ch of
  #97..#122 :
   result:=true;
...
{ is_numeric }
function is_numeric(ch : char8 ) : boolean;
begin
...
begin
 ...
 if ptr^ = #0 then
  raise svg_exception.Construct(char_ptr('parse_transform_args: Invalid syntax' ) );
 ...
 while ptrcomp(ptr ) < ptrcomp(end_ ) do
  if is_numeric(ptr^ ) then
   begin
    if na^ >= max_na then
     raise svg_exception.Construct(char_ptr('parse_transform_args: Too many arguments' ) );
...
begin
 ...
 if p = NIL then
  raise svg_exception.Construct(char_ptr('Couldn''t allocate memory for parser' ) );
 ...
 if not api_open_file(af ,fname ) then
  begin
   ...
   raise svg_exception.Construct(char_ptr(@msg[0 ] ) );
...
begin
 ...
 while char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ <> NIL do
  begin
   if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'style' ) = 0 then
...
begin
 ...
 while char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ <> NIL do
  ...
  // attributes (see 'else' branch).
   if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'d' ) = 0 then
    begin
 ...
 while char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ <> NIL do
  begin
   if not parse_attr(
    ...
    if StrComp(char_ptr(agg_basics.char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'points' ) = 0 then
     begin
      m_tokenizer.set_path_str(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

      if not m_tokenizer.next then
       raise svg_exception.Construct(char_ptr('parse_poly: Too few coordinates' ) );
      ...
      while m_tokenizer.next do
       begin
        ...
        if not m_tokenizer.next then
         raise svg_exception.Construct(char_ptr('parse_poly: Odd number of coordinates' ) );
...
begin
 ...
 while char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ <> NIL do
  begin
   if not parse_attr(
           ...
           agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ ) then
    begin
     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'x' ) = 0 then
      x:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'y' ) = 0 then
      y:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'width' ) = 0 then
      w:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'height' ) = 0 then
 ...
 if (w <> 0.0 ) and
  ...
  begin
   if w < 0.0 then
    raise svg_exception.Construct(char_ptr('parse_rect: Invalid width: ' ) );

   if h < 0.0 then
    raise svg_exception.Construct(char_ptr('parse_rect: Invalid height: ' ) );
 ...
 while char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ <> NIL do
  begin
   if not parse_attr(
           ...
           agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ ) then
    begin
     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'x1' ) = 0 then
      x1:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'y1' ) = 0 then
      y1:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'x2' ) = 0 then
      x2:=parse_double(agg_basics.char_ptr_ptr(ptrcomp(attr ) + (i + 1 ) * sizeof(char_ptr ) )^ );

     if StrComp(char_ptr(char_ptr_ptr(ptrcomp(attr ) + i * sizeof(char_ptr ) )^ ) ,'y2' ) = 0 then
...
begin
 while str^ <> #0 do
  begin
   if islower(str^ ) then
    if StrLComp(char_ptr(str ) ,'matrix' ,6 ) = 0 then
     inc(ptrcomp(str ) ,parse_matrix(str ) )
    else
     if StrLComp(char_ptr(str ) ,'translate' ,9 ) = 0 then
      inc(ptrcomp(str ) ,parse_translate(str ) )
     else
      if StrLComp(char_ptr(str ) ,'rotate' ,6 ) = 0 then
       inc(ptrcomp(str ) ,parse_rotate(str ) )
      else
       if StrLComp(char_ptr(str ) ,'scale' ,5 ) = 0 then
        inc(ptrcomp(str ) ,parse_scale(str ) )
       else
        if StrLComp(char_ptr(str ) ,'skewX' ,5 ) = 0 then
         inc(ptrcomp(str ) ,parse_skew_x(str ) )
        else
         if StrLComp(char_ptr(str ) ,'skewY' ,5 ) = 0  then
...
begin
 ...
 if na <> 6 then
  raise svg_exception.Construct(char_ptr('parse_matrix: Invalid number of arguments' ) );
...
begin
 ...
 if na = 1 then
  ...
  if na = 3 then
   ...
   raise svg_exception.Construct(char_ptr('parse_rotate: Invalid number of arguments' ) );
...
begin
 ...
 if StrComp(char_ptr(name ) ,'style' ) = 0 then
  parse_style(value )
 else
  if StrComp(char_ptr(name ) ,'fill' ) = 0 then
   if StrComp(char_ptr(value ) ,'none' ) = 0 then
    m_path.fill_none
   else
    begin
   ...
   if StrComp(char_ptr(name ) ,'fill-opacity' ) = 0 then
    m_path.fill_opacity(parse_double(value ) )
   else
    if StrComp(char_ptr(name ) ,'stroke' ) = 0 then
     if StrComp(char_ptr(value ) ,'none' ) = 0 then
      m_path.stroke_none
     else
      begin
     ...
     if StrComp(char_ptr(name ) ,'stroke-width' ) = 0 then
      m_path.stroke_width(parse_double(value ) )
     else
      if StrComp(char_ptr(name ) ,'stroke-linecap' ) = 0 then
       begin
        if StrComp(char_ptr(value ) ,'butt' ) = 0 then
         m_path.line_cap(butt_cap )
        else
         if StrComp(char_ptr(value ) ,'round' ) = 0 then
          m_path.line_cap(round_cap )
         else
          if StrComp(char_ptr(value ) ,'square' ) = 0 then
           m_path.line_cap(square_cap );

       end
      else
       if StrComp(char_ptr(name ) ,'stroke-linejoin' ) = 0 then
        begin
         if StrComp(char_ptr(value ) ,'miter' ) = 0 then
          m_path.line_join(miter_join )
         else
          if StrComp(char_ptr(value ) ,'round' ) = 0 then
           m_path.line_join(round_join )
          else
           if StrComp(char_ptr(value ) ,'bevel' ) = 0 then
            m_path.line_join(bevel_join );

        end
       else
        if StrComp(char_ptr(name ) ,'stroke-miterlimit' ) = 0 then
         m_path.miter_limit(parse_double(value ) )
        else
         if StrComp(char_ptr(name ) ,'stroke-opacity' ) = 0 then
          m_path.stroke_opacity(parse_double(value ) )
         else
          if StrComp(char_ptr(name ) ,'transform' ) = 0 then
           parse_transform(value )

        //else
        // if StrComp(char_ptr(el ) ,'<OTHER_ATTRIBUTES>' ) = 0 then
        //  begin
        //  end
        // ...
