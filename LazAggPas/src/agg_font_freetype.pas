...
//-----------------------------------------------font_engine_freetype_base
 font_engine_freetype_base = object(font_engine )
  ...
  // Set font parameters
   ...
   function  load_font(
              font_name : char_ptr; face_index : unsigned; ren_type : glyph_rendering;
              font_mem : char_ptr = NIL; font_mem_size : int = 0 ) : boolean;

   function  attach(file_name : char_ptr ) : boolean;
  ...
  // Interface mandatory to implement for font_cache_manager
   function  font_signature : char_ptr; virtual;
  ...
  // private
   ...
   function  find_face(name : char_ptr ) : int;
...
var
 ...
 tag : char8;  // current point's state
...
begin
 ...
 while n < outline.n_contours do
  begin
   ...
   tags :=char_ptr     (ptrcomp(outline.tags ) + first * sizeof(char8 ) );
   tag  :=FT_CURVE_TAG (tags^ );

  // A contour cannot start with a cubic control point!
   if tag = char8(FT_CURVE_TAG_CUBIC ) then
    begin
     result:=false;
  ...
  // check first point to determine origin
   if tag = char8(FT_CURVE_TAG_CONIC ) then
    begin
    // first point is conic control. Yes, this happens.
     if FT_CURVE_TAG(char_ptr(ptrcomp(outline.tags ) + last )^ ) = char8(FT_CURVE_TAG_ON ) then
      begin
       ...
       Do_Conic:
        if ptrcomp(point ) < ptrcomp(limit ) then
         begin
          ...
          if tag = char8(FT_CURVE_TAG_ON ) then
           begin
     ...
     // FT_CURVE_TAG_CUBIC
       ...
       begin
        if (ptrcomp(point ) + sizeof(FT_Vector ) > ptrcomp(limit ) ) or
           (FT_CURVE_TAG(char_ptr(ptrcomp(tags ) + 1 )^ ) <> char8(FT_CURVE_TAG_CUBIC ) ) then
         begin
          result:=false;
  ...
  begin
   ...
   if idx >= 0 then
    begin
     ...
     m_name    :=char_ptr(face_name_ptr(ptrcomp(m_face_names ) + idx * sizeof(face_name ) ).name );
    ...
    begin
     ...
     if m_last_error = 0 then
      begin
       ...
       StrCopy(
        char_ptr(face_name_ptr(ptrcomp(m_face_names ) + m_num_faces * sizeof(face_name ) ).name ) ,
        font_name );

       m_cur_face:=FT_Face_ptr_ptr(ptrcomp(m_faces ) + m_num_faces * sizeof(FT_Face_ptr ) )^;
       m_name    :=char_ptr(face_name_ptr(ptrcomp(m_face_names ) + m_num_faces * sizeof(face_name ) ).name );
      ...
      begin
       char_ptr(face_name_ptr(ptrcomp(m_face_names ) + m_num_faces * sizeof(face_name ) ).name ):=NIL;
...
{ ATTACH }
function font_engine_freetype_base.attach(file_name : char_ptr ) : boolean;
begin
 result:=false;
...
{ _NAME }
function font_engine_freetype_base._name : char_ptr;
begin
 result:=m_name;
...
{ FONT_SIGNATURE }
function font_engine_freetype_base.font_signature : char_ptr;
begin
 result:=char_ptr(m_signature.name );
...
begin
 if (m_cur_face <> NIL ) and
  ...
  begin
   ...
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%u,'  ,int(m_char_map ) );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,m_face_index );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,int(m_glyph_rendering ) );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d:'  ,m_resolution );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%dx'  ,m_height );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,m_width );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,int(m_hinting ) );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%d,'  ,int(m_flip_y ) );
   sprintf(char_ptr(ptrcomp(str ) + StrLen(char_ptr(str ) ) ) ,'%08X' ,gamma_hash );
   ...
   if (m_glyph_rendering = glyph_ren_outline ) or
    ...
    begin
     ...
     StrCat(char_ptr(m_signature.name ) ,buf );
...
{ FIND_FACE }
function font_engine_freetype_base.find_face(name : char_ptr ) : int;
var
 i : unsigned;
 n : face_name_ptr;
...
begin
 ...
 while i < m_num_faces do
  begin
   if StrComp(name ,char_ptr(n.name ) ) = 0 then
    begin
     result:=i;
