...
const
  TokenOperators = [ sum, diff, prod, divis, modulo, IntDiv, IntDIVZ, 
...
procedure ParseFunction( FunctionString: string;
          ...
          function CheckIntegerDIVZ(const s: string; var s1, s2: string): boolean; forward;
          {checks whether DIVZ is the primary TOperation in s}
          ...
          begin
            ...
            if CheckNumber(s1, FloatNumber) or
               ...
               CheckIntegerDIVZ(s1, s2{$IFNDEF Win32}^{$ENDIF}, s3{$IFNDEF Win32}^{$ENDIF}) or
                ...
                begin
                  ...
                  if not Result then
                    Result := CheckNegate(s1, s3{$IFNDEF Win32}^{$ENDIF}) or
                              ...
                              CheckIntegerDIVZ(s1, s3{$IFNDEF Win32}^{$ENDIF}, s4{$IFNDEF Win32}^{$ENDIF}) or
                    ...
                    begin
                      ...
                      if Result then
                        ...
                        Result := CheckAdd(s2, s3{$IFNDEF Win32}^{$ENDIF}, s4{$IFNDEF Win32}^{$ENDIF}) or
                                  ...
                                  CheckIntegerDIVZ(s2, s3{$IFNDEF Win32}^{$ENDIF}, s4{$IFNDEF Win32}^{$ENDIF}) or
          ...
          function CheckIntegerDIVZ(const s: string; var s1, s2: string): boolean;
          {checks whether 'DIV' is the primary TOperation in s}
          var
            s3, s4: TermString;
            i, j: integer;
          ...
          begin
            ...
            repeat
               ...
               j := pos('DIVZ', copy(s, i+1, length(s)-i));
              if j > 0 then
              begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          if CheckIntegerDIVZ(StartString, LeftString, RightString) then
                            Token := IntDIVZ
                          else
                          begin
